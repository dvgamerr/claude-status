...
func main() {
	...
	flex := tview.NewFlex().
		AddItem(tview.NewBox().SetBorder(true).SetDrawFunc(drawOverviewHeader), 0, 1, false).
		AddItem(tview.NewBox().SetDrawFunc(drawOSHeaderText), 14, 0, false).
	...
	go setTickerInterval(3*time.Second, getStatsOS)()
